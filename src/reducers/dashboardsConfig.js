...
export const sortFilterData = [
    {
        ...
        value: `${sortFilterKeyData[0].value} (${
            sortFilterDirectionData[0].value
        })`,
    },
    {
        id: `${sortFilterKeyData[0].id}_${sortFilterDirectionData[1].id}`,
        value: `${sortFilterKeyData[0].value} (${
            sortFilterDirectionData[1].value
        })`,
    },
    {
        id: `${sortFilterKeyData[1].id}_${sortFilterDirectionData[0].id}`,
        value: `${sortFilterKeyData[1].value} (${
            sortFilterDirectionData[0].value
        })`,
    },
    {
    ...
    },
    {
        ...
        value: `${sortFilterKeyData[2].value} (${
            sortFilterDirectionData[0].value
        })`,
    },
    {
    ...
    switch (action.type) {
        case actionTypes.SET_DASHBOARDSCONFIG_ISFETCHING:
            return validateReducer(
                action.value,
                DEFAULT_DASHBOARDSCONFIG_ISFETCHING
            );
        default:
            return state;
    }
...
const showFilter = (state = DEFAULT_DASHBOARDSCONFIG_SHOWFILTER, action) => {
    switch (action.type) {
        case actionTypes.SET_DASHBOARDSCONFIG_SHOWFILTER:
            return validateReducer(
                action.value,
                DEFAULT_DASHBOARDSCONFIG_SHOWFILTER
            );
        default:
            return state;
    }
...
const viewFilter = (state = DEFAULT_DASHBOARDSCONFIG_VIEWFILTER, action) => {
    switch (action.type) {
        case actionTypes.SET_DASHBOARDSCONFIG_VIEWFILTER:
            return validateReducer(
                action.value,
                DEFAULT_DASHBOARDSCONFIG_VIEWFILTER
            );
        default:
            return state;
    }
